...
connected_percent = 0.3
times_average = 3

t = TicToc()
runtimes_path = "runtimes/communities/" + infection_model + ".txt"
with open(runtimes_path, "w") as runtimes_output:
    runtimes_output.write("Runtimes (s) - max_community_size: " + str(max_community_size)
                          + " - connected_percent: " + str(connected_percent)
                          + " - times_average: " + str(times_average) + "\n")
...
for graph_num in range(1, graphs + 1):
    ...
    communities_path = "results/communities/" + infection_model + "/" + str(graph_num) + ".csv"
